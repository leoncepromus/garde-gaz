...
Users dial your USSD code (e.g. `*384*12345#` in sandbox) to check gas status or acknowledge an active leak **without a smartphone app or mobile data on the phone** — the session runs over the GSM network.
...
3. Save — you receive a **shared sandbox code** like `*384*12345#` (not `*131#`)
...
```ts
export const USSD_CODE = '*384*12345#';  // your assigned code from AT dashboard
```
...
| `*131#` does nothing | `*131#` is a placeholder — use the code **assigned by Africa's Talking** |
